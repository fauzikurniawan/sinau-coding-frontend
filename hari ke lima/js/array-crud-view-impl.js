...
function createRowACtion(item) {
    ...
    if (!item) {
        ...
        actions = '<button class="btn btn-danger" onclick="onEditOrCancel(this, \'CANCEL\')">Cancel</button>' +
            '<button class="btn btn-success" onclick="onSaveOrDelete(this, \'SAVE\')">Save</button>';
        ...
        actions = '<button class="button-edit" onclick="onEditOrCancel(this, \'EDIT\')">Edit</button>' +
            // Hanya tampil setelah klik edit
            '<button class="button-cancel" onclick="onEditOrCancel(this, \'CANCEL\')">Cancel</button>' +
            '<button class="button-save" onclick="onSaveOrDelete(this, \'SAVE\')">Save</button>' +
            // Selalu tampil
            '<button class="button-delete" onclick="onSaveOrDelete(this, \'DELETE\')">Delete</button>';

        // Membuat row dengan isi berdasarkan item
        rows = '<tr>' +
            '<td>' + item.id + '</td>' +
            // Tampilkan sebagai text biasa
            '<td class="readonly-content">' + item.content + '</td>' +
...
function onSaveOrDelete(element, type) {
    ...
    if (type == "SAVE") {
        ...
        // Modifikasi content dari oldItem dengan content dari inputan
        oldItem.content = content.value;
